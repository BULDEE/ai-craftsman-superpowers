...
|------|------------------------------|
| gate | `pre_verify` refuses the conclusion while LAYER/TS/PHP/SEC criticals remain; advisory findings surface once, without blocking. Opt-in `write_gate: on` also refuses a `write_file` or `patch` before it reaches disk, on SEC001 and LAYER001 only |
