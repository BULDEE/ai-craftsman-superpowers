...
metadata:
  hermes:
    tags: [craftsmanship, refactor]
    category: devops
    requires_toolsets: [terminal]
---

<!-- Generated by scripts/export-hermes-skills.sh from skills/refactor/SKILL.md. Edit the source, then re-run the export. -->
...
# the craftsman-refactor skill - Systematic Refactoring
...
3. **Tests red, absent, or too slow to trust** -> do NOT refactor blind. Route to `the craftsman-legacy skill cover` to build a characterization / golden-master net around the current behavior, then resume here. See `references/legacy-characterization-testing.md`.
...
- `references/refactoring-techniques.md` - the full Fowler catalog (Extract, Move, Split Phase, Pipeline, CQS)
- `references/refactoring-refactoring-campaigns.md` - hotspots (churn x complexity), prioritization
- `references/refactoring-mikado-method.md` - safe multi-file change via discovery/delivery
- `references/legacy-legacy-techniques.md` - seams, subclass & override, wrap & sprout
- `references/legacy-characterization-testing.md` - golden master net before refactoring untested code
- `references/legacy-strangler-fig.md` - branch-by-abstraction, incremental cutover
