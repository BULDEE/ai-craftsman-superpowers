...
```bash
sqlite3 "$(bash "${HOME}/.hermes/plugins/craftsman/bin/craftsman-runtime" metrics)" "SELECT rule, file_pattern, action, context FROM corrections WHERE timestamp > datetime('now','-7 days') ORDER BY timestamp DESC LIMIT 10;" 2>/dev/null || echo "No recent corrections"
```
