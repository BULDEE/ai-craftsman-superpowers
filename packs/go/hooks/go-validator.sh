...
_GO_PACK_SCAN_WARNED=""
_go_structure_lines() {
    local file="$1" out status=0
    out=$(python3 "$_GO_STRUCTURE_PY" "$file" 2>/dev/null) || status=$?
    if [[ "$status" -ne 0 && -z "$out" && -z "${_GO_PACK_SCAN_WARNED}" ]]; then
        _GO_PACK_SCAN_WARNED=1  # craftsman-ignore: WARN-SH001 - once per process, local would reset it
        echo "craftsman: go structure scan did not run (exit ${status}), files are not clean, they are unread; set CLAUDE_PLUGIN_ROOT" >&2
    fi
    printf '%s\n' "$out"
}
