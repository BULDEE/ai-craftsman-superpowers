...
Use the **Bash** tool to run this EXACT command (do NOT modify or simplify it):
```
bash ~/.claude/craftsman-set-verified.sh
```
This wrapper was generated by the SessionStart hook with the correct resolved paths.
If the wrapper is missing, fall back to this command:
```
