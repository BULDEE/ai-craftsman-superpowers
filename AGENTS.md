...
Codex plugin that transforms Codex into a disciplined Senior Software Craftsman. DDD, Clean Architecture, TDD methodology enforced through hooks, commands, agents, and a rules engine.
...
- A plugin option reaches a hook as `CLAUDE_PLUGIN_OPTION_<KEY>` with the key
  ...
  for four releases.
- All commands MUST have `description`, `effort` in frontmatter. `effort` is Codex's own frontmatter key, not project metadata: it overrides the session effort level, so only `low`, `medium`, `high`, `xhigh`, `max` are valid.
...
Records every violation fix users make, injects correction trends at next session start, and promotes recurring fixes into human-reviewed learned skills. SQLite-backed feedback loop that progressively teaches Codex the exact patterns your codebase rejects. Cross-file pattern detection suggests project-wide fixes when 3+ files share the same violation. Differentiator: the learning loop is coupled to the rules engine, so what is learned comes from violations that same engine enforces in hooks and in CI, not from a separate model of what good code looks like.
...
- Level 2: Static analysis - PHPStan, ESLint. When tools installed and trusted,
  under a budget of 15s per file and 30s per project.
- Level 3: Architecture - deptrac, dependency-cruiser. Same budget.
...
adapters/           → host agent runtimes (a different axis from ci/adapters/)
  hermes/           → Nous Research Hermes: pre_verify hook, Codex wrapper
```

Three front-ends over one core: `hooks/` for Codex, `ci/craftsman-ci.sh`
...
- `.Codex-plugin/marketplace.json` → root `version` + plugin `version`
- `ci/craftsman-ci.sh` → `VERSION=`
- `AGENTS.md` → `**Current version:**` (this file; the checklist used to omit
...
Then tag with `Codex plugin tag --push`, which produces the
