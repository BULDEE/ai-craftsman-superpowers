...
The clock is held still with `mock.patch`, which is NOT a seam. It reaches into
the module and replaces a name, so it breaks the moment the import moves. It is
here because the net has to exist before the code can be touched, and this is
the cheapest thing that makes today's behaviour reproducible. Step 2 replaces
it with a real seam.
"""
...
class CharacterizationTest(unittest.TestCase):
    ...
    def price(self, *args, today=TUESDAY):
        buffer = io.StringIO()
        with mock.patch("pricing.datetime") as clock:
            clock.now.return_value = today
            with redirect_stdout(buffer):
                total = PricingService().total(*args)
